...
讀取所有商品的「日」三關價分析結果，依「月份」分頁彙整成一份 Excel，
分析完成後自動寄送到指定信箱。
...
def collect_all_daily_rows() -> pd.DataFrame:
    """跑一次所有商品的「日」分析，把每天每個商品的一列資料收集起來"""
    cfg = load_symbols()
    ...
    for item, source in items:
        code = item['code']
        name = item['name']
        a = analyze_one(code, name, source, 'daily')
        if not a:
            continue
        df = a['df'].copy()
        df['code'] = code
...
def write_monthly_workbook(full: pd.DataFrame, out_path: Path):
    wb = Workbook()
    ...
    months = sorted(full['year_month'].unique())
    ...
    for ym in months:
        ...
        sub = full[full['year_month'] == ym].sort_values(['date', 'code'])
...
def send_email(attachment_path: Path):
    ...
    msg['From'] = smtp_user
    msg['To'] = mail_to
    msg.set_content(f'附件為 {today} 三關價每日彙總報表（依月份分頁，全商品彙整），系統自動寄送。')
...
def main():
    full = collect_all_daily_rows()
    if full.empty:
        print('沒有任何商品資料，略過月彙總報表')
        return

    write_monthly_workbook(full, REPORT_PATH)
