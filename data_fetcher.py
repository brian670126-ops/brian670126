...
def fetch_yfinance_ohlcv(ticker: str, start: str = "2018-01-01",
    ...
    台股代號如果用 .TW（上市）抓不到，會自動改試 .TWO（上櫃）再抓一次，
    因為很多中小型IC設計/材料股是在櫃買中心掛牌，容易搞混。"""
    import yfinance as yf

    def _try_fetch(t: str) -> pd.DataFrame:
        last_err = None
        for attempt in range(retries):
            try:
                df = yf.download(t, start=start, end=end, progress=False, auto_adjust=False)
                if df.empty:
                    raise ValueError(f"{t} 沒有抓到任何資料")
                df = df.reset_index()
                if isinstance(df.columns, pd.MultiIndex):
                    df.columns = [c[0] if c[1] == '' or c[1] == t else c[0] for c in df.columns]
    ...
    try:
        return _try_fetch(ticker)
    except Exception as first_err:  # noqa: BLE001
        # 台股數字代號 .TW 抓不到時，自動改試 .TWO（上櫃）
        if ticker.endswith(".TW"):
            alt_ticker = ticker[:-3] + ".TWO"
            try:
                return _try_fetch(alt_ticker)
            except Exception:  # noqa: BLE001
                pass
        raise RuntimeError(
            f"抓取 {ticker} 失敗，重試 {retries} 次後仍失敗（含嘗試 .TWO 後綴）："
            f"{first_err}；請確認代號是否正確（尤其台股需要 .TW / .TWO 後綴）"
        )
