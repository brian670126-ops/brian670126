...
def parse_tx(text):
    close   = fv(text, r'closed at ([\d,\.]+)', r'收盤[|：:]\s*([\d,\.]+)')
    b3      = fv(text, r'B3[|：:\s]+([\d,\.]+)', r'[Rr]esistance 3[|：:\s]+([\d,\.]+)')
    b2      = fv(text, r'B2[|：:\s]+([\d,\.]+)', r'[Rr]esistance 2[|：:\s]+([\d,\.]+)')
    b1      = fv(text, r'B1[|：:\s]+([\d,\.]+)', r'[Rr]esistance 1[|：:\s]+([\d,\.]+)')
    m_val   = fv(text, r'\bM[|：:\s]+([\d,\.]+)', r'[Mm]idpoint[^0-9]*([\d,\.]{5,})')
    s1      = fv(text, r'S1[|：:\s]+([\d,\.]+)', r'[Ss]upport 1[|：:\s]+([\d,\.]+)')
    s2      = fv(text, r'S2[|：:\s]+([\d,\.]+)', r'[Ss]upport 2[|：:\s]+([\d,\.]+)')
    s3      = fv(text, r'S3[|：:\s]+([\d,\.]+)', r'[Ss]upport 3[|：:\s]+([\d,\.]+)')
    ...
    chg     = fv(text, r'漲跌[|：:\s]*([-\d,\.]+)', r'([-\d,\.]+)\s*\([-\d\.]+%\)')
    amp     = fv(text, r'振幅[|：:\s]*([\d,\.]+)')
    return dict(close=close, b3=b3, b2=b2, b1=b1, m=m_val, s1=s1, s2=s2, s3=s3,
...
def gate_table(d, label='明日預測三關價'):
    ...
    html += '<tr style="background:#f8f9fa;"><th style="padding:6px 10px;text-align:left;color:#7f8c8d;font-size:12px;">關卡</th><th style="padding:6px 10px;text-align:left;color:#7f8c8d;font-size:12px;">數值</th><th style="padding:6px 10px;text-align:left;color:#7f8c8d;font-size:12px;">距收盤</th><th style="padding:6px 10px;font-size:12px;color:#7f8c8d;">位置</th></tr>'
    for key, lbl, color, val in rows:
        if not val:
            continue
        diff = round(val - close, 2) if close else 0
        diff_str = ('+' if diff >= 0 else '') + '{:,.0f}'.format(diff)
        near = abs(diff) < 150 if close else False
        ...
        html += (
            ...
            '<td style="padding:6px 10px;font-family:monospace;font-size:15px;font-weight:bold;">' + '{:,.2f}'.format(val) + '</td>'
            '<td style="padding:6px 10px;color:' + ('#e74c3c' if diff > 0 else '#27ae60') + ';font-size:13px;">' + diff_str + '</td>'
            '<td style="padding:6px 10px;width:120px;">'
            '<div style="background:#ecf0f1;border-radius:3px;height:10px;">'
            '<div style="background:' + color + ';height:100%;width:' + str(bar_pct) + '%;border-radius:3px;"></div>'
            '</div></td>'
            '</tr>'
        )
    html += '</table>'
...
def five_day_table(rows):
    ...
    html += '<tr style="background:#f8f9fa;"><th style="padding:5px 8px;text-align:left;">日期</th><th style="padding:5px 8px;">開</th><th style="padding:5px 8px;">高</th><th style="padding:5px 8px;">低</th><th style="padding:5px 8px;">收</th><th style="padding:5px 8px;">漲跌</th><th style="padding:5px 8px;">量</th></tr>'
    prev_close = None
    for r in rows:
        try:
...
def trend_analysis(rows):
    ...
    recent_3 = closes[-3:]
    recent_trend = '持續上漲' if all(recent_3[i] > recent_3[i-1] for i in range(1,3)) else \
                   '持續下跌' if all(recent_3[i] < recent_3[i-1] for i in range(1,3)) else '震盪整理'
...
def strategies_html(tx, prob):
    ...
    all_strategies = [
        ('S01','M附近做多→目標B1','多方M~B1主戰場策略', close and m_val and abs(close - m_val) < 200 and tx.get('bull')),
        ('S02','B1附近反彈失敗做空→目標M','B1壓力反轉策略', close and b1 and abs(close - b1) < 200),
        ('S03','突破B1後拉回做多→目標B2','B1突破後回測進場', close and b1 and close > b1),
        ('S04','收盤守S1觀察次日再多','S1支撐確認多', close and s1 and close > s1 and abs(close - s1) < 300),
        ('S05','破S2立即清空多單','S2止損守則', close and s2 and close < s2),
        ('S06','連續上漲後M附近減碼','趨勢過熱降倉', tx.get('weak_w') or tx.get('mid_w')),
        ('S07','量縮守M整理等待','低量整理等噴出', tx.get('amp') and tx.get('amp', 1000) < 500),
        ('S08','強警訊出現先觀望','警訊先蹲再跳', tx.get('strong') or tx.get('mid_w')),
        ('S09','週線多方+日線拉回做多','周日共振進場', tx.get('bull')),
        ('S10','弱警訊+多方：半倉試多','弱警訊輕倉策略', tx.get('weak_w') and tx.get('bull')),
        ('S11','早盤高開S1以上 → 空手','高開不追多', close and m_val and close > b1 if b1 else False),
        ('S12','跌破M首日觀望不做空','M失守等確認', close and m_val and close < m_val),
        ('S13','B2以上不追多，等回M','強多不追高守則', close and b2 and close > b2 if b2 else False),
        ('S14','多商品同步多方加碼','國際共振加碼', tx.get('bull')),
        ('S15','美股夜盤大漲→次日縮小漲幅觀察','外資套利效應', True),
        ('S16','融資增加+量增→確認多方延續','籌碼確認策略', tx.get('bull')),
        ('S17','M~B1區間來回當沖','箱型操作策略', close and m_val and b1 and m_val < close < b1 if (m_val and b1) else False),
        ('S18','前高突破後回測支撐做多','突破回測經典型態', tx.get('bull') and not tx.get('strong')),
        ('S19','三連陰後守M反彈做多','情緒K線反轉', not tx.get('bull')),
        ('S20','原油大跌觀察台股供應鏈','跨商品聯動觀察', True),
        ('S21','盤整末期量縮S1附近做多','量縮整理末升段', tx.get('amp') and tx.get('amp', 1000) < 600),
        ('S22','強警訊後次日量增是否守M定多空','警訊後量能確認', tx.get('strong') or tx.get('mid_w')),
    ]

    matched = [(code, name, desc) for code, name, desc, cond in all_strategies if cond]
    unmatched = [(code, name, desc) for code, name, desc, cond in all_strategies if not cond]

    html = '<table style="border-collapse:collapse;width:100%;font-size:13px;">'
    html += '<tr style="background:#f8f9fa;"><th style="padding:6px 8px;text-align:left;width:50px;">代碼</th><th style="padding:6px 8px;text-align:left;">策略名稱</th><th style="padding:6px 8px;text-align:left;">說明</th><th style="padding:6px 8px;width:70px;">適用</th></tr>'
    ...
    for code, name, desc in matched:
        html += (
            ...
            '<td style="padding:6px 8px;text-align:center;">✅ 適合</td>'
            '</tr>'
        )
    for code, name, desc in unmatched:
        html += (
            ...
            '<td style="padding:6px 8px;text-align:center;color:#bbb;">—</td>'
            '</tr>'
        )
    html += '</table>'
...
def teacher_notes(tx):
    ...
    notes.append(('D-02','位置決定意義，同一根K線在不同位置意義完全不同',
        '目前收盤在 {} 區間，進場前先確認位置再論多空。'.format(
            'M~B1 核心安全區' if (m_val and b1 and m_val < close < b1) else
            'B1以上（警戒勿追多）' if (b1 and close > b1) else
            'M以下（需等確認）' if (m_val and close < m_val) else '整理區')))
    ...
    for code, rule, remind in notes:
        html += (
            '<div style="border-left:3px solid #f39c12;padding:10px 14px;margin-bottom:12px;background:#fffbf0;border-radius:0 6px 6px 0;">'
            '<div style="font-size:12px;color:#e67e22;font-weight:bold;margin-bottom:4px;">依據 ' + code + '</div>'
            '<div style="font-size:13px;color:#7f8c8d;font-style:italic;margin-bottom:6px;">「' + rule + '」</div>'
...
def build_html(tx_text, tx_w_text, t5f_text, latest_text, tx_rows, t5f_rows):
    ...
    html = (
        ...
        '<style>'
        'body{font-family:"Helvetica Neue",Arial,"Microsoft JhengHei",sans-serif;background:#f0f2f5;margin:0;padding:16px 0;}'
        '.wrap{max-width:760px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.12);}'
        '.header{background:linear-gradient(135deg,#1a1a2e 0%,#16213e 50%,#0f3460 100%);padding:32px 36px;color:#fff;}'
        ...
        '.section h2{margin:0 0 16px;font-size:17px;color:#1a1a2e;border-left:5px solid #e74c3c;padding-left:12px;font-weight:bold;}'
        '.section h3{margin:12px 0 8px;font-size:14px;color:#2c3e50;}'
        '.kv{display:flex;flex-wrap:wrap;gap:12px;margin-bottom:14px;}'
        '.kv-item{background:#f8f9fa;border-radius:8px;padding:12px 18px;min-width:110px;border:1px solid #ecf0f1;}'
        '.kv-item .label{font-size:11px;color:#95a5a6;margin-bottom:5px;text-transform:uppercase;letter-spacing:.5px;}'
        ...
        '.prob-fill-bull{background:linear-gradient(90deg,#e74c3c,#c0392b);height:100%;border-radius:6px;transition:width .5s;}'
        '.prob-fill-bear{background:linear-gradient(90deg,#27ae60,#1e8449);height:100%;border-radius:6px;}'
        '.tag{display:inline-block;padding:3px 10px;border-radius:4px;font-size:12px;font-weight:bold;}'
        '.bull{background:#fdecea;color:#c0392b;}'
        '.bear{background:#e8f8f0;color:#1e8449;}'
        '.warn{background:#fef9e7;color:#b7950b;}'
        'table{width:100%;border-collapse:collapse;}'
        'th{background:#f8f9fa;padding:7px 10px;text-align:left;font-size:12px;color:#7f8c8d;}'
        'td{padding:6px 10px;}'
        '.footer{background:#1a1a2e;padding:20px 36px;font-size:12px;color:#7f8c8d;text-align:center;}'
    ...
    html += (
        '<div class="header">'
        '<h1>📊 蘭老師 TX 深度日報</h1>'
        '<p>' + TODAY + '（星期' + TODAY_WEEKDAY + '）｜台指期 · 台灣50 · 22策略 · 深度分析版</p>'
        '</div>'
    )
    ...
    html += '<div class="kv">'
    html += '<div class="kv-item"><div class="label">資料日期</div><div class="value" style="font-size:14px;">' + tx.get('date','-') + '</div></div>'
    html += '<div class="kv-item"><div class="label">收盤價</div><div class="value">' + close_str + '</div></div>'
    html += '<div class="kv-item"><div class="label">日線方向</div><div class="value" style="font-size:14px;">' + direction_badge(tx.get('bull')) + '</div></div>'
    html += '<div class="kv-item"><div class="label">警訊</div><div class="value" style="font-size:12px;">' + alert_badge(tx.get('strong'), tx.get('mid_w'), tx.get('weak_w')) + '</div></div>'
    html += '<div class="kv-item"><div class="label">週線方向</div><div class="value" style="font-size:14px;">' + direction_badge(tx_w.get('bull')) + '</div></div>'
    html += '</div>'
    ...
    html += (
        ...
        '</div>'
        '<p style="font-size:12px;color:#95a5a6;margin:4px 0 0;">評分依據：日線方向、警訊強度、與M/B1/S2相對位置、近期走勢</p>'
        '</div>'
    )
    ...
    html += '<div class="kv">'
    html += '<div class="kv-item"><div class="label">收盤價</div><div class="value">' + t5f_close_str + '</div></div>'
    html += '<div class="kv-item"><div class="label">日線方向</div><div class="value" style="font-size:14px;">' + direction_badge(t5f.get('bull')) + '</div></div>'
    html += '<div class="kv-item"><div class="label">警訊</div><div class="value" style="font-size:12px;">' + alert_badge(t5f.get('strong'), t5f.get('mid_w'), t5f.get('weak_w')) + '</div></div>'
    html += '</div>'
    ...
    html += (
        ...
        '</div>'
        '<div class="prob-bar"><div class="prob-fill-bull" style="width:' + str(t5f_prob) + '%;"></div></div>'
        '</div>'
    )
    ...
    html += '<p style="font-size:13px;color:#555;margin-top:10px;padding:10px;background:#f8f9fa;border-radius:6px;">' + trend_tx + '</p>'
    html += '<h3 style="margin-top:16px;">台灣50期貨（T5F）近5日 K 線</h3>'
    html += five_day_table(five_days_t5f[-5:])
    html += '<p style="font-size:13px;color:#555;margin-top:10px;padding:10px;background:#f8f9fa;border-radius:6px;">' + trend_t5f + '</p>'
    html += '</div>'

    # 4. 22策略
    html += '<div class="section"><h2>🎯 四、22大策略框架 — 今日適合策略（' + str(matched_count) + '個符合）</h2>'
    html += '<p style="font-size:13px;color:#7f8c8d;margin:0 0 12px;">依當前盤勢條件自動篩選，綠色底色為今日適合操作策略：</p>'
    html += strat_html
    html += '</div>'
    ...
    if commodity_rows:
        ...
        html += '<table><tr><th>商品</th><th>收盤</th><th>漲%</th><th>方向</th><th>警訊</th><th>區間</th></tr>'
        for row in commodity_rows[:18]:
            if len(row) >= 6:
                ...
                alert_color = '#e74c3c' if '強' in alert_txt else '#f39c12' if '中' in alert_txt else '#f1c40f' if '弱' in alert_txt else '#27ae60'
                html += (
                    '<tr style="border-bottom:1px solid #f0f0f0;">'
                    '<td style="font-weight:bold;">' + row[1] + '</td>'
                    '<td style="font-family:monospace;">' + row[3] + '</td>'
                    '<td>' + row[2] + '</td>'
    ...
    html += (
        ...
        '<p>本報告依三關價系統自動產生，僅供技術面參考。新資訊進來時，判斷需動態調整（貝葉斯思維）。</p>'
        '<p>—— 蘭老師 AI 分身 · ' + TODAY + '</p>'
        '</div>'
        '</div></body></html>'
    )
...
def send_email(html_body):
    ...
    if not smtp_user or not smtp_pass or not mail_to:
        print('ERROR: SMTP env not set'); sys.exit(1)
